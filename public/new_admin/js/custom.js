...
function showConfirm(title=null,content=null,acceptCallback, cancelCallback=null, type='blue') {
	$.alert({
		icon: 'fa fa-warning',
		title: title,
		content : content,
...
function showAjaxFormValidation(errorData) {
    ...
    for(datas in errorResp.errors){
        errors += ' <li><img src="'+path+'new_admin/images/danger.svg" class="icon_svg" /> ' + errorResp.errors[datas][0] + '</li>';
    }
...
function throwValidationError(errors) {
    ...
    var error = '';
    for(data of errors){
        error += ' <li><img src="'+path+'new_admin/images/danger.svg" class="icon_svg" /> ' + data + '</li>';
    }
    console.log(error);
    $(document.body).find('.form_validation_error').show();
    $(document.body).find( '.error_list' ).html( error ); //appending to a <div id="form-errors"></div> inside form
    $('html, body').animate({scrollTop:0},'50');
