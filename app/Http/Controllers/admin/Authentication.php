...
class Authentication extends AdminBaseController
{
    ...
    public function index()
    {
		...
		$data['patient']      =   Patient::count('id');
		$data['collector']    =   Collector::count('id');
		$data['test']         =   PathTest::count('id');
		$data['amount']       =   Billing::whereMonth('created_at', date('m'))->whereYear('created_at', date('Y'))->sum('total_amount');
        $user_id = Session::get('admin_details')['id'];
        return view('admin/dashboard/admin_dashboard',$data,['dashboard_data' => $dashboard_data,'profile_image' => $this->profile_image,'parent_menu' => $this->parent_menu, 'sub_menu' =>$this->sub_menu]);
    }
    /**
